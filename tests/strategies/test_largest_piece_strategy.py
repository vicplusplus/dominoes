...
class TestLargestPieceStrategy(unittest.TestCase):
    def test_select_largest_piece(self):
        ...
        self.assertEqual(
            move.domino,
            Domino(5, 4),
            "The strategy did not select the largest piece available.",
        )
    ...
    def test_no_possible_moves(self):
        ...
        board.add(
            Domino(0, 1), "right"
        )  # Assuming this sets the board to a state incompatible with the hand

        move = strategy.select_move(hand, board)

        # Expecting None since no moves can be played
        self.assertIsNone(
            move, "The strategy should return None when no moves can be played."
        )
