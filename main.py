...
def simulate_games(strategy_combination):
    ...
    for _ in range(NUM_GAMES):
        ...
        for _, player in enumerate(game.players):
            scores[int(player.name)] += player.score()
    return strategy_combination, wins, [score / 100 for score in scores]
...
def main():
    strategies = [
        ...
        AggregateMinimaxStrategy(2),
        AggregateMinimaxStrategy(4),
    ...
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        ...
        for future in futures:
            strategy_combination, wins, avg_scores = future.result()
            results[strategy_combination] = (wins, avg_scores)
    ...
    for strategy_combination, (wins, scores) in results.items():
        for player_index, strategy in enumerate(strategy_combination):
            ...
            total_scores[strategy_idx] += scores[player_index]
    ...
    for i, strategy in enumerate(strategies):
        print(
            f"Strategy {str(strategy)}: Total Wins = {total_wins[i]}, Total Score = {total_scores[i]:.2f}"
        )
